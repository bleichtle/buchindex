...
module.exports = async ({ book_path, frequency_path }) => {
	...
	const cover_id = book.metadata.cover
	if (cover_id) {
		try {
			const cover_buffer = await book.getImageAsync(cover_id)
			console.log(cover_buffer)
			book_data.Cover = 'data:image/jpeg;base64,' + cover_buffer[0].toString('base64')
	...
	book_data.Chapters = book_data.Chapters
		.filter(chapter => chapter.TotalTokens > (average_chapter_length * 0.05))
